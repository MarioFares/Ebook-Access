...
class Ui_MainWindow(QtWidgets.QWidget):
    ...
    def show_app_settings(self):
        ...
        self.settings_dialog.apply_pushButton.clicked.connect(lambda: self.apply_style())
        self.settings_dialog.save_pushButton.clicked.connect(lambda: self.save_style())
        self.settings_dialog.ok_pushButton.clicked.connect(lambda: self.ok_apply_style())
        self.settings_dialog.load_pushButton.clicked.connect(lambda: self.load_style())
        self.settings_dialog.activate_theme_pushButton.clicked.connect(lambda: self.activate_theme())
        self.settings_dialog.defaults_pushButton.clicked.connect(
            lambda: self.settings_dialog.saved_themes_comboBox.setCurrentText("Default"))
        self.settings_dialog.defaults_pushButton.clicked.connect(lambda: self.apply_style())
        self.settings_dialog.saved_themes_comboBox.removeItem(0)
        self.settings_dialog.saved_themes_comboBox.insertItem(0, "")
        self.settings_dialog.saved_themes_comboBox.removeItem(1)
        self.settings_dialog.saved_themes_comboBox.insertItem(1, "Default")
        self.refresh_themes()
        self.dialog.exec_()
    ...
    def refresh_themes(self):
        with self.connect:
            ...
            for theme in themes:
                self.settings_dialog.saved_themes_comboBox.addItem(theme[0])

    def activate_theme(self):
        theme_name = self.settings_dialog.saved_themes_comboBox.currentText()
        with self.connect:
    ...
    def load_style(self):
        ...
        if theme is None:
            ...
            self.settings_dialog.labels_font_comboBox.setCurrentText(theme[1])
            self.settings_dialog.labels_color_comboBox.setCurrentText(theme[2])
            self.settings_dialog.labels_font_color_comboBox.setCurrentText(theme[3])
            self.settings_dialog.title_font_comboBox.setCurrentText(theme[4])
            self.settings_dialog.title_color_comboBox.setCurrentText(theme[5])
            self.settings_dialog.title_font_color_comboBox.setCurrentText(theme[6])
            self.settings_dialog.menubar_font_comboBox.setCurrentText(theme[7])
            self.settings_dialog.menubar_color_comboBox.setCurrentText(theme[8])
            self.settings_dialog.menubar_font_color_comboBox.setCurrentText(theme[9])
            self.settings_dialog.statusbar_font_comboBox.setCurrentText(theme[10])
            self.settings_dialog.statusbar_color_comboBox.setCurrentText(theme[11])
            self.settings_dialog.statusbar_font_color_comboBox.setCurrentText(theme[12])
            self.settings_dialog.button_font_comboBox.setCurrentText(theme[13])
            self.settings_dialog.button_color_comboBox.setCurrentText(theme[14])
            self.settings_dialog.button_radius_comboBox_2.setCurrentText(str(theme[15]))
            self.settings_dialog.button_font_color_comboBox_2.setCurrentText(theme[16])
            self.settings_dialog.inputbox_font_comboBox.setCurrentText(theme[17])
            self.settings_dialog.inputbox_color_comboBox_2.setCurrentText(theme[18])
            self.settings_dialog.inputbox_radius_comboBox.setCurrentText(str(theme[19]))
            self.settings_dialog.inputbox_font_color_comboBox.setCurrentText(theme[20])
            self.settings_dialog.combobox_font_comboBox.setCurrentText(theme[21])
            self.settings_dialog.combobox_color_comboBox.setCurrentText(theme[22])
            self.settings_dialog.combobox_radius_comboBox.setCurrentText(str(theme[23]))
            self.settings_dialog.combobox_font_color_comboBox.setCurrentText(theme[24])
            self.settings_dialog.background_color_comboBox.setCurrentText(theme[25])
    ...
    def apply_active_theme(self):
        with self.connect:
            ...
            theme = self.cursor.fetchone()
        if theme is None:
            MainWindow.setStyleSheet("background_color: white")
            self.main_menu.setStyleSheet("background-color: rgb(216, 216, 216);")
            self.statusBar.setStyleSheet("background-color: rgb(216, 216, 216);")
            self.window_title_label.setStyleSheet("background-color: white")
        else:
            ...
            self.main_menu.setStyleSheet(f"font-family: {theme[7]};\n"
                                         f"color: {theme[9]};\n"
                                         f"background-color: {menubar_color};")
            self.statusBar.setStyleSheet(f"font-family: {theme[10]};\n"
                                         f"color: {theme[12]};\n"
    ...
    def apply_style(self):
        if self.settings_dialog.saved_themes_comboBox.currentText() == "Default":
            MainWindow.setStyleSheet("background_color: white")
            self.main_menu.setStyleSheet("background-color: rgb(216, 216, 216);")
            self.statusBar.setStyleSheet("background-color: rgb(216, 216, 216);")
            self.window_title_label.setStyleSheet("background-color: white")
        else:
            if self.settings_dialog.statusbar_color_comboBox.currentText() == "Light Grey":
                statusbar_color = "rgb(216, 216, 216)"
            else:
                statusbar_color = self.settings_dialog.statusbar_color_comboBox.currentText()
            if self.settings_dialog.menubar_color_comboBox.currentText() == "Light Grey":
                menubar_color = "rgb(216, 216, 216)"
            else:
                menubar_color = self.settings_dialog.menubar_color_comboBox.currentText()
            self.window_title_label.setStyleSheet(
                f"font-family: {self.settings_dialog.title_font_comboBox.currentText()};\n"
                f"color: {self.settings_dialog.title_font_color_comboBox.currentText()};\n"
                f"border-radius: {self.settings_dialog.button_radius_comboBox_2.currentText()}px;\n"
                f"background-color: {self.settings_dialog.title_color_comboBox.currentText()};")
            self.main_menu.setStyleSheet(f"font-family: {self.settings_dialog.menubar_font_comboBox.currentText()};\n"
                                         f"color: {self.settings_dialog.menubar_font_color_comboBox.currentText()};\n"
                                         f"background-color: {menubar_color};")
            self.statusBar.setStyleSheet(f"font-family: {self.settings_dialog.statusbar_font_comboBox.currentText()};\n"
                                         f"color: {self.settings_dialog.statusbar_font_color_comboBox.currentText()};\n"
                                         f"background-color: {statusbar_color};"
                                         )
            MainWindow.setStyleSheet(
                f"#centralwidget{{background-color: {self.settings_dialog.background_color_comboBox.currentText()};}}\n"
                f"QLabel{{font-family: {self.settings_dialog.labels_font_comboBox.currentText()};\n"
                f"color: {self.settings_dialog.labels_font_color_comboBox.currentText()};\n"
                f"border-width: 2px;\n"
                f"border-radius: {self.settings_dialog.button_radius_comboBox_2.currentText()}px;\n"
                f"background-color: {self.settings_dialog.labels_color_comboBox.currentText()}}}\n"

                f"QLineEdit{{font-family: {self.settings_dialog.inputbox_font_comboBox.currentText()};\n"
                f"color: {self.settings_dialog.inputbox_font_color_comboBox.currentText()};\n"
                "border-width: 2px;\n"
                f"border-radius: {self.settings_dialog.inputbox_radius_comboBox.currentText()}px;\n"
                f"background-color: {self.settings_dialog.inputbox_color_comboBox_2.currentText()}}}\n"

                f"QComboBox{{font-family: {self.settings_dialog.combobox_font_comboBox.currentText()};\n"
                f"color: {self.settings_dialog.combobox_font_color_comboBox.currentText()};\n"
                "border-width: 2px;\n"
                f"border-radius: {self.settings_dialog.combobox_radius_comboBox.currentText()}px;\n"
                f"background-color: {self.settings_dialog.combobox_color_comboBox.currentText()}}}\n"

                f"QPushButton{{font-family: {self.settings_dialog.button_font_comboBox.currentText()};\n"
                f"color: {self.settings_dialog.button_font_color_comboBox_2.currentText()};\n"
                "border-width: 2px;\n"
                f"border-radius: {self.settings_dialog.button_radius_comboBox_2.currentText()}px;\n"
                f"background-color: {self.settings_dialog.button_color_comboBox.currentText()}}}\n"

                f"QListWidget{{background-color: {self.settings_dialog.labels_color_comboBox.currentText()};\n"
                f"color: {self.settings_dialog.labels_font_color_comboBox.currentText()};\n"
                f"border-radius: {self.settings_dialog.combobox_radius_comboBox.currentText()}px;\n"
                f"font-family: {self.settings_dialog.combobox_font_comboBox.currentText()}}};\n"
            )

    def save_style(self):
        if self.settings_dialog.theme_name_lineEdit.text() == '':
            self.error_box("You must specify a name for your theme.")
            return 0
        with self.connect:
            self.cursor.execute("SELECT name FROM settings")
            themes = [theme[0] for theme in self.cursor.fetchall()]
            if self.settings_dialog.theme_name_lineEdit.text() in themes:
                self.error_box("There is already a theme by that name.")
                return 0
            self.cursor.execute("""INSERT INTO settings (name,
                                                       ...
                                                       active)  VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,
                                                                        ?,?,?)""",
                                (self.settings_dialog.theme_name_lineEdit.text(),
                                 self.settings_dialog.labels_font_comboBox.currentText(),
                                 self.settings_dialog.labels_color_comboBox.currentText(),
                                 self.settings_dialog.labels_font_color_comboBox.currentText(),
                                 self.settings_dialog.title_font_comboBox.currentText(),
                                 self.settings_dialog.title_color_comboBox.currentText(),
                                 self.settings_dialog.title_font_color_comboBox.currentText(),
                                 self.settings_dialog.menubar_font_comboBox.currentText(),
                                 self.settings_dialog.menubar_color_comboBox.currentText(),
                                 self.settings_dialog.menubar_font_color_comboBox.currentText(),
                                 self.settings_dialog.statusbar_font_comboBox.currentText(),
                                 self.settings_dialog.statusbar_color_comboBox.currentText(),
                                 self.settings_dialog.statusbar_font_color_comboBox.currentText(),
                                 self.settings_dialog.button_font_comboBox.currentText(),
                                 self.settings_dialog.button_color_comboBox.currentText(),
                                 self.settings_dialog.button_radius_comboBox_2.currentText(),
                                 self.settings_dialog.button_font_color_comboBox_2.currentText(),
                                 self.settings_dialog.inputbox_font_comboBox.currentText(),
                                 self.settings_dialog.inputbox_color_comboBox_2.currentText(),
                                 self.settings_dialog.inputbox_radius_comboBox.currentText(),
                                 self.settings_dialog.inputbox_font_color_comboBox.currentText(),
                                 self.settings_dialog.combobox_font_comboBox.currentText(),
                                 self.settings_dialog.combobox_color_comboBox.currentText(),
                                 self.settings_dialog.combobox_radius_comboBox.currentText(),
                                 self.settings_dialog.combobox_font_color_comboBox.currentText(),
                                 self.settings_dialog.background_color_comboBox.currentText(),
                                 0
                                 ))
        self.refresh_themes()
